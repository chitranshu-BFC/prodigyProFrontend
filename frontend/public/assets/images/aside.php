...
<aside id="menu" class="sidebar">
   <ul class="nav metis-menu" id="side-menu">
      ...
      <li class="menu-item-<?php echo $item->id; ?>">
         ...
         <ul class="nav nav-second-level collapse" aria-expanded="false">
            <?php foreach($item->children as $submenu){
               ...
               ?>
                <li  class="menu-item-<?php echo "$submenu->id";?> <?php if(!isset($submenu->sub_children)) {}?>"><a href="<?php echo admin_url($submenu->url); ?>" aria-expanded="false">

               <?php if(!empty($submenu->icon)){ ?>
                  <i class="<?php echo $submenu->icon; ?> menu-icon"></i>
                  <?php } ?>
                  <?php echo _l("$submenu->name"); ?>
                  <?php if($submenu->sub_children){ ?>
                  <span class="fa arrow"></span>
                  <?php } ?>
                  </a>
                <?php if(isset($submenu->sub_children)){
                  foreach($submenu->sub_children as $subChild){
                ?>
                 <ul class="nav <?php if(!isset($subChild->inner_sub_children)) { echo "nav-third-level";}?> collapse" aria-expanded="false">
                    <li <?php if(isset($subChild->inner_sub_children)) { echo 'style="margin-left: 10px;"';} ?> class="menu-item-<?php echo $subChild->id;?>"><a href="<?php echo admin_url($subChild->url); ?>">
                    <?php if(!empty($subChild->icon)){ ?>
                      <i class="<?php echo $subChild->icon; ?> menu-icon"></i>
                      <?php } ?>

                    <?php echo _l("$subChild->name");?>
                     <?php if($subChild->inner_sub_children){ ?>
                    <span class="fa arrow"></span>
                     <?php } ?>
                     </a>
                     <?php if(isset($subChild->inner_sub_children)){
                      foreach($subChild->inner_sub_children as $InnerSubChild){
                    ?>
                      <ul class="nav nav-fourth-level collapse" aria-expanded="false">
                        <li class="sub-menu-item-<?php echo $InnerSubChild->id; ?>"><a href="<?php echo admin_url($InnerSubChild->url); ?>">
                        <?php if(!empty($InnerSubChild->icon)){ ?>
                          <i class="<?php echo $InnerSubChild->icon; ?> menu-icon"></i>
                        <?php } ?>
                        <?php echo _l("$InnerSubChild->name");?>
                          </a>
                      </li>
                    </ul>
                  <?php }} ?>
      ...
      <?php } ?>
      <?php if((is_admin()) && $this->app->show_setup_menu() == true){ ?>
